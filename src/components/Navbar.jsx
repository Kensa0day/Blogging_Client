...
const MobileMenu = ({ user, signOut }) => {
    ...
    return (
        <div className="flex">
            <button onClick={toggleMenu} className="lg:hidden p-2 text-gray-600 hover:text-gray-800">
            <svg
            ...
            {user?.token ? (
              ...
              <Link to='/sign-in'>
                <Button
                  label='Sign In'
                  styles='flex items-center justify-center bg-black dark:bg-purple-600 text-white dark:text-white text-white px-4 py-1.5 rounded-full'
                />
              </Link>
            )}
          </div>
        <ThemeSwitch/>
        <span className='cursor-pointer text-xl font-semibold dark:text-white' onClick={toggleMenu}><AiOutlineClose /></span>
    </div>
                )
            }
        </div>
        ...
        <div className='flex gap-2 items-center cursor-pointer'>
          {user?.token ? (
            ...
            <Link to='/sign-in'>
              <Button
                label='Sign in'
                styles='flex items-center justify-center bg-black dark:bg-purple-600 text-white dark:text-white px-4 py-1.5 rounded-full'
              />
            </Link>
          )}
        </div>
